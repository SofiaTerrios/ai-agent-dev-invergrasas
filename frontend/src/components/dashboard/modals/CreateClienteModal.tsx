...
import { CREATE_CLIENTE_MUTATION, type CreateClienteInput } from "@/graphql/clientes";
...
type FormValues = CreateClienteInput;
...
export default function CreateClienteModal({
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await graphqlRequest(CREATE_CLIENTE_MUTATION, { input: values }, token);
      setValues({ nombre: "", contacto: "", telefono: "", correo: "", empresa_id: "" });
      onSuccess();
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : "Error al crear cliente");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="w-full max-w-md rounded-lg border border-[#DDE8E2] bg-white shadow-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="border-b border-[#DDE8E2] px-6 py-4 sticky top-0 bg-white">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-[#17251F]">Nuevo Cliente</h2>
              <button
                type="button"
                onClick={onClose}
          ...
          <form onSubmit={onSubmit} className="space-y-4 px-6 py-4">
            ...
            <div>
              ...
              {errors.empresa_id && (
                <p className="mt-1 text-xs text-[#D63A2F]">{errors.empresa_id}</p>
              )}
            </div>
